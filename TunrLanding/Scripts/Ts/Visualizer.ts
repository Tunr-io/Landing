...
class Visualizer {
    ...
    private showBG(images: string[]) {
        this.currentBG = new VisualBG(images);
    ...
    private showVisual() {
        ...
        this.currentVisual = new LineScrollVisual(Visualizer.textLines);
...
class Visual extends createjs.Container {
    ...
    constructor(textLines: Array<string>) {
...
class LineScrollVisual extends Visual {
    ...
    public show(callback: () => void): void {
        ...
        var fieldSelection = this.textLines.slice(); // Get a copy of the fields
        ...
        for (var i = 0; i < numLines; i++) {
            ...
            var str: string = fieldSelection.splice(Math.floor(Math.random() * fieldSelection.length), 1)[0];
